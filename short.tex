...
As shown in Table~\ref{tab:security_results}, both methods achieved an MIA AUC score of approximately \textbf{0.50}. An AUC of 0.50 indicates that an attacker performs no better than random guessing when attempting to distinguish training data from unseen data. This confirms that SPA preserves the same strict privacy guarantees as the baseline. 

Notably, SPA exhibited significantly higher \textbf{Update Entropy} (2.67) compared to the padding baseline (1.47). This suggests that SPA updates contain richer information density within the subspace, whereas padding results in sparse, low-entropy updates artifacts. Crucially, this increased information density does not lead to data leakage, as evidenced by the neutral MIA score.
...
\midrule
Homo $r=4$ & $57.8\pm0.7$ & $12.85\pm0.07$ & $0.571\pm0.007$ & 8.2 & 0.5019 & 45.2 \\
Homo $r=8$ & $61.2\pm0.7$ & $12.22\pm0.08$ & $0.605\pm0.007$ & 6.8 & 0.5021 & 90.4 \\
Hetero-Pad & $60.1\pm0.7$ & $12.59\pm0.08$ & $0.593\pm0.007$ & 7.5 & 0.5023 & 67.8 \\
\textbf{Hetero-SPA} & \textbf{63.8$\pm$0.7} & \textbf{11.94$\pm$0.08} & \textbf{0.632$\pm$0.007} & \textbf{5.8} & \textbf{0.5024} & \textbf{67.8} \\
\bottomrule
\end{tabular}
\end{table*}

SPA demonstrated the lowest hallucination rate (5.8\%), indicating greater stability in generating valid outputs. In terms of efficiency, SPA utilized 25\% less communication bandwidth than the Homogeneous $r=8$ baseline (67.8 MB vs. 90.4 MB) while achieving higher accuracy and maintaining equivalent privacy standards.
...
\caption{Efficiency trade-offs showing accuracy versus training time (left) and communication cost (right). SPA achieves the best accuracy-efficiency balance, performing better than Homogeneous $r=8$ while using 25\% less communication bandwidth and 8.7\% less training time.}
...
The cumulative energy plot reveals that SPA captures \textbf{82.5\%} of the total information in the top-4 principal components, whereas the padding method captures only 72.8\%. This implies that when projecting to rank-4 for low-resource clients, SPA preserves significantly more global knowledge.
...
Second, \textbf{Spectral Denoising without Information Loss}: Neural network updates inherently contain noise. By truncating the singular values, SPA effectively acts as a denoising filter. Our security analysis confirms that this filtering does not degrade privacy; the MIA AUC remains at 0.50 (random baseline), proving that the retained "signal" is generalizable knowledge rather than memorized user data. The higher update entropy observed in SPA (2.67 vs. 1.47) indicates that the method successfully concentrates rich, semantic information into the available subspace, avoiding the sparsity artifacts inherent in zero-padding.
...
\textbf{Privacy-Preserving Efficiency}: SPA delivers higher accuracy while reducing bandwidth requirements by 25\%. Importantly, our MIA results demonstrate that this efficiency gain does not come at the cost of user privacy, making SPA suitable for sensitive domains like healthcare or finance.
...
We propose that heterogeneity in federated learning should be embraced. By utilizing geometry-aware aggregation methods like SPA, diverse device capabilities can be unified into a robust learning system that outperforms homogeneous configurations without compromising the privacy of individual participants.
